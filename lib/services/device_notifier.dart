...
Future<void> notifyDevice(BluetoothDevice device, DeviceInfo deviceInfo) async {
  ...
  NotificationService().showOutOfRangeNotification(deviceInfo.name);
  ...
  await Future.delayed(
    Duration(seconds: 10),
  ); // buzzer bakal auto mati dalam 10 detik
...
Future<void> writeToDevice(BluetoothDevice device, List<int> values) async {
  ...
  BluetoothCharacteristic? buzzerCharacteristic = service?.characteristics
      .firstWhereOrNull(
